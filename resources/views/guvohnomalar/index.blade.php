...
{{-- Filtrlar --}}
@php $hasFilters = request()->filled('q') || request()->filled('razryad') || request()->filled('date_from') || request()->filled('date_to'); @endphp
<form method="GET" action="{{ route('guvohnomalar.index') }}" class="card mt-5 p-4">
    <div class="flex flex-wrap items-end gap-3">
        <div class="min-w-[14rem] flex-1">
            <span class="text-xs text-slate-500 dark:text-navy-300">Qidirish</span>
            <label class="relative mt-1 flex">
                <input name="q" value="{{ request('q') }}" placeholder="Raqam, F.I.O., mutaxassislik"
        ...
        <div class="w-24">
            <span class="text-xs text-slate-500 dark:text-navy-300">Razryad</span>
            <input name="razryad" value="{{ request('razryad') }}"
        ...
        @if($hasFilters)
        <a href="{{ route('guvohnomalar.index') }}" class="btn border border-slate-300 text-slate-600 hover:bg-slate-150 dark:border-navy-450 dark:text-navy-200 dark:hover:bg-navy-500">
...
<p class="mt-3 text-xs text-slate-400 dark:text-navy-300">
    Jami: <span class="font-medium text-slate-600 dark:text-navy-100">{{ $guvohnomalar->total() }}</span> ta guvohnoma
</p>
            ...
            <tbody>
                ...
                <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                    ...
                    <td class="px-4 py-3">
                        {{ $g->fullNameOz() }}
                        @if($g->fullNameRu())
                        <br><span class="text-xs text-slate-400">{{ $g->fullNameRu() }}</span>
                        @endif
                    </td>
                ...
                <tr><td colspan="7" class="text-center py-8 text-slate-400">
                    {{ $hasFilters ? 'Filtrga mos guvohnoma topilmadi.' : 'Guvohnomalar yo\'q.' }}
                </td></tr>
                @endforelse
            </tbody>
        </table>
